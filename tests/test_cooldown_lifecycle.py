"""Cooldown-cycle lifecycle through the real student scripts (simulated backend).

device -> cycle -> wiring era -> runs: start a cycle, measure (stamped), hand-add a
mapping snapshot (era moves), end the cycle (runs stamp "" again). Mirrored scripts —
LCHQMDriver runs identical copies, so this coverage transfers.
"""
...
def _run(env: dict, script: str, *args: str) -> subprocess.CompletedProcess:
...
def test_cooldown_lifecycle(tmp_path):
    ...
    proc = _run(env, "cooldown.py", "start", "cd1", "--fridge", "TestFridge", "--packaging", "PCB v1")
    assert proc.returncode == 0, proc.stderr
    assert _run(env, "cooldown.py", "start", "cd2").returncode != 0

    # a run is stamped with the cycle (no mapping yet -> empty wiring era)
    proc = _run(env, "run_experiment.py", "resonator_spectroscopy", "--qubits", "q0")
    assert proc.returncode == 0, proc.stderr
    ...
    # find by cycle through the student query script
    listed = _run(env, "find_runs.py", "--cooldown", "cd1")
