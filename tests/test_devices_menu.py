"""devices.py — the Tier-1 discovery menu (config + registries only, no instrument).
...
def _run(config: Path, user: str | None = None) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, str(REPO / "scripts" / "devices.py")],
...
def test_menu_lists_backends_and_selection_hint(tmp_path):
    ...
    out = _run(config, user='backend = "qblox"\n').stdout
