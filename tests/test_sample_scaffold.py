"""sample.py — the add-a-sample scaffold (prints snippets, never edits shared files).

Mirrored script; LCHQMDriver runs an identical copy.
"""
...
def _run(config: Path | None, *args: str) -> subprocess.CompletedProcess:
    env = {**os.environ, "SCQO_USER_CONFIG": "none"}
    if config is not None:
        env["SCQO_CONFIG"] = str(config)
    return subprocess.run(
        [sys.executable, str(REPO / "scripts" / "sample.py"), *args],
...
def test_scaffold_prints_snippets_and_creates_folder(tmp_path):
    ...
    proc = _run(config, "new", "chipC", "--backend", "qblox", "--instrument", "cluster0",
    ...
    assert "[cluster0]" not in out.split("devices.toml")[1] or "instruments.toml" not in out
    # known instrument -> NO instruments.toml scaffold section
    assert "is not\n   registered yet" not in out
    assert "cooldown.py start" in out  # next steps
...
def test_unknown_instrument_gets_registry_snippet(tmp_path):
    proc = _run(_config(tmp_path), "new", "chipD", "--backend", "qm", "--instrument", "fridgeX")
    assert proc.returncode == 0, proc.stderr
...
def test_existing_name_warns(tmp_path):
    config = _config(tmp_path)
    assert _run(config, "new", "chipC").returncode == 0
    proc = _run(config, "new", "chipC")  # folder now exists from the first call
...
def test_requires_data_root_and_checklist_works(tmp_path):
    ...
    proc = _run(config, "new", "chipC")
    assert proc.returncode != 0
    assert "data_root" in proc.stderr

    checklist = _run(config)  # no command -> the self-documenting checklist
